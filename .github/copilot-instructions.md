...
- **`CleanupManager`** (`content/CleanupManager.mjs`): Age-based tab closure and memory optimization. Also supports **auto-unload of idle tabs** (`unloadStaleTabs()`) based on `autoUnloadDelay`. Memory reporting uses `ChromeUtils.requestProcInfo()` and `Services.sysinfo`. Cleanup age calculations prefer `tab.lastAccessed`, then Firefox Places history visit timestamps when a tab has been restored after restart. Automatic cleanup/auto-unload never touches essential or pinned tabs. Memory optimization may discard essential/pinned tabs when under memory pressure.
